...
- **게시판 3-스택 모범사례**(별도 페이지, 실제 게시판): <http://localhost:8081/demo/board.html> — 하나의 `board_post` 를 **Spring Data JPA / jOOQ / Spring Data JDBC** 세 방식으로. 각 탭이 **목록·검색·페이징·상세·글쓰기·수정·삭제**를 갖춘 실제 게시판이며, 셋 다 계층형(Controller→Service→Repository) + Record DTO + MapStruct + Bean Validation + 표준 응답/페이징/에러(공통 라이브러리) 적용. 엔드포인트 `/demo/board/{jpa,jooq,jdbc}`. **AI 코딩 참조 가이드** → [260702.004.BOARD_SAMPLE_GUIDE.md](260702.004.BOARD_SAMPLE_GUIDE.md).
- **보안 테스트**(별도 페이지, 보안 하드닝 검증 기준): <http://localhost:8081/demo/security.html> — 위배 시나리오(미인증 401·권한부족 403·X-User-Id 스푸핑·JWT audience 불일치·오류 정보 미노출·권한 없는 토큰·보안 응답 헤더)를 유발하고 **실시간 보안 감사 로그**(`ai.mutuus.common.security.audit`)를 확인한다. 보호 엔드포인트 `/api/secure/*`·`/api/admin/*`. **가이드** → [260702.006.SECURITY_HARDENING.md](260702.006.SECURITY_HARDENING.md).
